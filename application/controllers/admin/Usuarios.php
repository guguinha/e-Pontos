...
class Usuarios extends CI_Controller {
	...
	public function inserir()
	{
		...
		$this->form_validation->set_rules('txt-user','Login','required|min_length[3]|is_unique[usuario.login]');
		...
		if($this->form_validation->run() == FALSE){
			...
			if($this->modelusuarios->adicionar($nome,$user,$senha)){
	...
	public function salvar_alteracoes(){
		...
		$usuario = $this->modelusuarios->retorne_login($id);
		$user = $this->input->post('txt-user');	
		var_dump($usuario);
		if($usuario[0]->login != $user){
			$this->form_validation->set_rules('txt-user','Login','required|min_length[3]|is_unique[usuario.login]');
		}
		if($this->form_validation->run() == FALSE){
			$this->alterar(md5($id));
			...
			//$user = $this->input->post('txt-user');			
			//$id = $this->input->post('txt-id');
			if($this->modelusuarios->alterar($nome,$user,$id)){
				redirect(base_url('admin/usuarios'));
			}else{
				echo "Houve um erro no sistema!";
			}
	...
	public function salvar_nova_senha(){
		...
		if($this->form_validation->run() == FALSE){
			...
			$senha = $this->input->post('txt-senha');
			//$id = $this->input->post('txt-id');
			if($this->modelusuarios->alterar_senha($senha,$id)){
				redirect(base_url('admin/usuarios'));
			}else{
				echo "Houve um erro no sistema!";
			}
